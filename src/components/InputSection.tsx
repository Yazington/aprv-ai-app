...
const InputSection = ({ input, setInput, handleSend, isLoading = false }: Props) => {
    ...
    <div className="flex w-full transform-gpu py-4">
      <div className="relative mx-auto flex w-full max-w-2xl items-center justify-center">
        <div className="group relative w-full">
          ...
          <input
            ref={textareaRef}
            className="relative w-full rounded-xl border-none bg-lightBg2 px-5 py-3.5 text-[15px] shadow-lg transition-all duration-300 placeholder:text-gray-600 focus:outline-none focus:ring-2 focus:ring-lightBg4 disabled:cursor-not-allowed disabled:opacity-50 dark:bg-darkBg2 dark:text-gray-100 dark:placeholder:text-gray-400 dark:focus:bg-darkBg3 dark:focus:ring-darkBg4"
        ...
        <button
          onClick={handleSend}
          className="group absolute right-2 flex h-10 w-10 items-center justify-center overflow-hidden rounded-xl bg-lightBg4 text-textPrimary shadow-lg transition-all duration-300 hover:scale-105 hover:shadow-xl disabled:cursor-not-allowed disabled:opacity-50 disabled:hover:transform-none dark:bg-darkBg4 dark:text-textSecondary"
          disabled={!isInputNotEmpty || isLoading}
        >
